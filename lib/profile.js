$(document).ready(function () {
    ...
    var postLoadQueue = [];
    var isModuleReady = false;
    function postLoadHook (event, data) {
        postLoadQueue.push({
            event: event,
            data: data
        });
    }
    $(window).on('action:topic.loaded', postLoadHook);
    $(window).on('action:posts.loaded', postLoadHook);
    ...
    require(['viteBridge', 'qrcode'], function (module, QRCode) {
        ...
        function renderPostLoadQueue() {
            if (postLoadQueue.length) {
                postLoadQueue.forEach(function (item) {
                    renderPostQR(item.event, item.data);
                })
            }
        }
        ...
        function openModal() {
            ...
            $('.qrcode-popover').popover('destroy');
